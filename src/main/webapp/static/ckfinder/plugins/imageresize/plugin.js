...
CKFinder.addPlugin('imageresize', {
    ...
    uiReady: function (api) {
            ...
            return {
                ...
                onShow: function () {
                        ...
                        if (CKFinder.env.ie6Compat) {
                            if (width > height)
                                previewImg.setStyles({
                                    width: 100 + 'px',
                                    height: Math.round(height / ( width / 100 )) + 'px'
                                });
                            else
                                previewImg.setStyles({
                                    height: 100 + 'px',
                                    width: Math.round(width / ( height / 100 )) + 'px'
                                });
                        }
                        else {
                    ...
                    var updateImgDimension = function (width, height) {
                        ...
                        CKFinder.tools.setTimeout(function () {
                            switchLockRatio(dialog, 'check');
                        }, 0, dialog);
                    };
        ...
        api.addFileContextMenuOption({
                label: api.lang.Imageresize.contextMenuName,
                command: 'resizeImage'
            }, function (api, file) {
